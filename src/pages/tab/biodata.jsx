...
// Import store agar data sinkron ke Navbar & permanen
import useAuthStore from '../../stores/useAuthStore'; 

export default function BiodataTab() {
  // Ambil data user dan fungsi update dari store global
  const { user, updateUser } = useAuthStore();
  
  // State untuk mengontrol kemunculan dan isi Modal Pop-up
  const [modal, setModal] = useState({ open: false, type: '', title: '', value: '' });
  const fileInputRef = useRef(null);

  // Fungsi untuk membuka modal sesuai kategori (Nama, Tgl Lahir, Kelamin)
  const openModal = (type, title, currentVal) => {
    setModal({ open: true, type, title, value: currentVal || '' });
  };

  // Fungsi untuk menyimpan perubahan ke Store Global
  ...
  // PERUBAHAN FUNGSI: Menggunakan FileReader (Base64) agar foto tidak hilang saat refresh
  const handleFileChange = (e) => {
    ...
    if (file) {
      ...
      reader.onloadend = () => {
        // Update foto di store dengan format Base64 agar permanen
  ...
  return (
    <div className="flex flex-col md:flex-row gap-12 animate-in fade-in duration-500">
      ...
      {/* ================= MODAL POP-UP (Logic & UI Tokopedia) ================= */}
      {modal.open && (
          ...
          <div className="bg-white w-full max-w-[450px] rounded-2xl shadow-2xl p-8 relative animate-in zoom-in duration-300">
            ...
            <div className="space-y-4">
              ...
              <button 
                ...
                className={`w-full py-3 rounded-xl font-black uppercase text-xs tracking-widest mt-8 transition-colors ${
                  modal.value ? 'bg-green-500 text-white shadow-lg shadow-green-100' : 'bg-gray-100 text-gray-400 cursor-not-allowed'
                }`}
              >
                Simpan
              </button>
